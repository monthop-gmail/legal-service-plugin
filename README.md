...
| `list_leads` | Odoo | ดู Lead ทั้งหมด — stdio only (admin) |
...
# ตั้งค่า HF_TOKEN + Ingest กฎหมาย 57 มาตรา เข้า ChromaDB (ครั้งแรก)
...
```bash
# 1. ตั้งค่า HF_TOKEN (สำหรับ download embedding model)
export HF_TOKEN=your_huggingface_token

# 2. Ingest กฎหมายเข้า ChromaDB (ครั้งแรก)
docker compose up chromadb ingest

# 3. Start core services (ChromaDB + RAG + MCP)
docker compose up -d
...
เพิ่มใน `.mcp.json` (project-level) หรือ `~/.claude/settings.json`:
...
| `rag-legal` | `legal-th-rag` | 8080 | default | RAG API (Python + multilingual-e5, shared hf-cache) |
...
├── .mcp.json                       # MCP server config (type: http, auto-discovered)
├── .claude-plugin/
